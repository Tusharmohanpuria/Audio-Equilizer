...
def process_audio(audio, q=1.4, noise_reduce=False, noise_sample_duration=1):
    global p_val
...
##########################################################################################
    ...
    # Define the parameters for the Wiener filter
    if noise_reduce:
        noise_sample_size = int(sample_rate * noise_sample_duration)
        noise_std = 0.05 * np.max(data)
        noise = np.zeros((noise_sample_size, channels))
        for i in range(channels):
            noise[:, i] = np.random.normal(0, noise_std, noise_sample_size)
        noise_psd = np.mean(np.abs(np.fft.fft(noise, axis=0)) ** 2, axis=1)
        signal_psd = np.mean(np.abs(np.fft.fft(data, axis=0)) ** 2, axis=1)
        alpha = noise_psd / signal_psd
        alpha[alpha > 1] = 1
        alpha[alpha < 1e-5] = 1e-5
...
##########################################################################################
            ...
            # Apply the Wiener filter to the filtered signal
            if noise_reduce:
                filtered[:, j] = signal.wiener(filtered[:, j], mysize=(3,3)) * alpha[j]
